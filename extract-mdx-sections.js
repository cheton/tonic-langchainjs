...
const rootdir = './tonic-ui/packages/react-docs/';
const sourceFilepath = `${rootdir}pages/components/select/index.page.mdx`;
const content = fs.readFileSync(sourceFilepath, 'utf8');

// Split markdown into sections by heading level 2
let sections = splitMarkdownSections(content, { min: 2, max: 2 });
...
const docs = sections.map((section) => {
  ...
  const metadata = {
    type: 'mdx',
    source: path.relative(rootdir, sourceFilepath),
